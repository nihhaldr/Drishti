...
export const AlertPanel = () => {
  ...
  const saveToCache = (alertsData: Alert[]) => {
    try {
      const cacheData: CacheData = {
        alerts: alertsData.filter(alert => alert.is_active),
        // Only cache active alerts
        timestamp: Date.now(),
        version: 1
      };
  ...
  useRealtime([{
    ...
    callback: payload => {
      ...
      if (payload.eventType === 'INSERT') {
        ...
        const updatedAlerts = alerts.map(alert => alert.id === updatedAlert.id ? updatedAlert : alert).filter(alert => alert.is_active); // Remove resolved alerts
  ...
  if (loading) {
    return <div className="w-80 bg-white border-l border-gray-200 p-4 shadow-sm">
        <div className="text-gray-600">Loading emergency alerts...</div>
      </div>;
  }
  return <div className="w-80 bg-white border-l border-gray-200 p-4 overflow-y-auto shadow-sm">
      ...
      <div className="space-y-3">
        {alerts.length === 0 ? <Card className="bg-gray-50 border-gray-200 p-4">
            <div className="text-center text-gray-500">
              <Bell className="w-8 h-8 mx-auto mb-2 opacity-50" />
              <p>No active emergency alerts</p>
            </div>
          </Card> : alerts.map(alert => <Card key={alert.id} className="bg-red-50 border-red-200 p-3 hover:shadow-md transition-shadow">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {alert.message && <p className="text-xs text-gray-600 mb-2 line-clamp-2">
                      {alert.message}
                    </p>}
                  
                  <div className="flex items-center justify-between text-xs text-gray-500 mb-3">
                    {alert.location_name && <div className="flex items-center gap-1">
                  ...
                  {alert.latitude && alert.longitude && <div className="text-xs text-gray-400 mb-2">
                      📍 {alert.latitude.toFixed(6)}, {alert.longitude.toFixed(6)}
                    </div>}
                  
                  <div className="flex gap-2 flex-wrap">
                    
                    {!alert.acknowledged_at && <Button size="sm" variant="outline" className="text-xs h-6 border-gray-300 text-gray-600 hover:text-blue-600 hover:border-blue-300" onClick={() => handleAcknowledge(alert.id)}>
                        Acknowledge
                      </Button>}
                    <Button size="sm" variant="outline" className="text-xs h-6 border-gray-300 text-gray-600 hover:text-green-600 hover:border-green-300" onClick={() => handleResolve(alert.id)}>
                      Resolve
                    </Button>
                  </div>
                </div>
      ...
      <Button className="w-full mt-4 bg-red-600 hover:bg-red-700 text-white shadow-md" onClick={loadAlerts}>
